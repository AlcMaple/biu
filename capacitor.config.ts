...
// 留空时走默认离线包行为，Android / iOS 生产构建不受影响。
...
const config: CapacitorConfig = {
  ...
  plugins: {
    // CapacitorHttp 已被 http-native.ts 直接调用；这里启用是为了让其同时
    // patch 全局 fetch/XHR，确保第三方库的请求也走原生通道（绕过 WebView CORS）
    CapacitorHttp: { enabled: true },
    // 启用 Android / iOS 原生 cookie 桥接：让 platform.getCookie/setCookie 可读写 WebView 的
