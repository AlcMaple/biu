...
## 2026-09-26 fix(web): 浏览器标签页显示网站 Logo

复用现有绿色电视 Logo 作为 SVG 网站图标，由 Rsbuild 输出图标文件并在页面头部自动添加引用，让浏览器标签页显示 Biu 标识。已通过 `pnpm build:web`，并核对构建后的图标内容及 HTML 引用。

## 2026-09-19 fix(player): 切歌只取分集信息，避免大合集详情拖停连续播放

**问题与证据**：在用户 Chrome 中核对两个 Sentry 超时分组，均出现“view 预取超时 → 正式切歌 view 再超时 → 未处理 Promise 异常”；9 月 17 日事件与生产日志时间一致，9 月 18 日最新事件同样命中 view。反代对应响应各约 1.68 MB。对故障视频进行一次只读取样，完整详情 1,679,044 字节，主要体积来自播放不需要的 ugc_season；轻量 pagelist 响应 364 字节，分集数组与完整详情完全一致。HTTP 200 不代表浏览器在超时前收到完整正文。同刻的收藏关系请求是独立、已捕获异常的旁路，并不阻塞播放。

**关键修改**：队列中的歌曲已有标题、封面和作者，预取、自动切歌及手动重试只补充分集，改走 pagelist，不再重复下载合集详情。保留原队列属性、分 P 顺序和时长；缺少 aid 时使用已有 BV 转 AV 工具补齐。首次点播需要完整资料的路径保持不变。源准备成功才开播，旧请求不抢播；等待切歌时改为 removeAttribute("src") + load() 卸载旧源，避免 Chrome 对空 src 触发虚假的媒体错误。不增加无限自动重试，也不延长全局超时

## 2026-09-12 fix(web): 修复部分搜索歌曲获取播放链接失败

**效果**：部分歌曲只有 PCDN 主源和区域运营商 CDN 备用源时，Web BFF 不再把可用备用地址一起清空；仍只返回绑定会话的短期不透明媒体 token，继续拒绝 PCDN、任意地址及非标准端口。

**证据与链路**：生产回传日志中两次失败均包含正常音轨编码/码率，但音频地址和候选列表为空。对其中一首低频核查，playurl 返回 PCDN 主源和 `cn-…bilivideo.com` 备用源；旧白名单只认 UPOS，重写时两者都被过滤。该备用源在实际运行主机的一次 1 KiB Range 请求返回 `206`、`Content-Range: bytes 0-1023/2001439`。问题取决于歌曲分配的 CDN，不是所有搜索点播都会失败，也不证明歌单中的其他歌曲永远不受影响。

**关键修改**：`web-server/routes.ts` 增加区域/运营商/双节点编号格式校验；URL 协议、凭据、端口和媒体路径检查保持不变，token 注册、解析和重定向复用同一个校验。未增加自动重试，未开放任意域名代理，也未暴露签名媒体地址。

**验证**：旧规则下新增的主机校验、备用地址重写与 Range 集成回归 3 项失败；修改后代理/播放队列相关 56 项通过，ESLint 和 `pnpm run build:web` 通过。测试覆盖伪装域名、异常端口、凭据 URL、非媒体路径、PCDN 拒绝、会话隔离及真实本地 HTTP Range 转发。未部署生产；源站 Range 成功与本地 BFF 集成通过不等于线上浏览器完整播放已验收。
