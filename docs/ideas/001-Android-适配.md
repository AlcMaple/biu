...
- [进行中] 实机回归：v2.3.0 PC 两项（桌面歌词关闭重开位置 / 打开无白闪）+ Android Splash UI 视觉效果
...
- **PC**：跑当前 macOS Biu，开桌面歌词，拖到屏幕左上角关闭，再次打开
  验证位置回到左上；反复开关桌面歌词 10 次以上确认首帧不白闪。
- **Android**：`pnpm build:android` → `pnpm open:android` → 起模拟器，看
  ...
  版本号），路由分叉是否走对。
- **Android 持久化全链路**（顺带）：登录 → token 写 Preferences → 杀进程
  重启 → 自动登录 → refreshCookie 跑通，确认 cookie jar 在 WebView
  重启后保留。
