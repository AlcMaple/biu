...
- **输入**：MP3/WAV 等音频文件 + 纯文本歌词（多行）
- **输出**：标准 LRC 文件（每行带 `[mm:ss.xx]` 时间戳）
...
export function dtwAlign(
  ...
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      const c = charCost(realChars[i - 1], asrChars[j - 1].char);
      dp[i][j] = Math.min(
        dp[i - 1][j - 1] + c,         // 匹配
...
export function buildLrc(
  ...
  for (const line of lines) {
    ...
    const lineChars = Array.from(line).filter(c => c.trim() && !isPunctuation(c));
    if (lineChars.length === 0) {
      continue;
    }
    
    const time = charTimes[charIdx];
    if (time !== undefined) {
      lrcLines.push({ time, text: line });
    }
    charIdx += lineChars.length;
  }

  return lrcLines.map(l => `${formatLrcTime(l.time)}${l.text}`).join('\n');
}
...
export async function alignLyrics(
  ...
  // 4. 提取真实歌词字符序列（标点和空白都过滤）
  const realChars = lyricsText
    .split(/\r?\n/)
    .flatMap(line => Array.from(line))
    .filter(c => c.trim() && !isPunctuation(c));

  // 5. DTW 对齐
  const mapping = dtwAlign(realChars, asrChars);
  ...
  // 7. 生成 LRC
  return buildLrc(lyricsText, charTimes);
}
```
...
3. 实现 DTW + LRC 生成模块，用 mock 数据单测
4. 接入 OSS 上传
5. 最后集成到 Biu 的 UI 流程
6. 边做边记录每首测试歌曲的对齐质量，调整 DTW 的 `INSERT_COST` / `DELETE_COST`
