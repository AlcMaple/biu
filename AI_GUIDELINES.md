...
- ❌ 在 HTTP/1.1 的域名上用长轮询做实时通知
  后果：真实事故——Chromium 每域名只有 6 条连接，长轮询独占一条长达 25s，同一域名的其他请求（同步的 pull/push）排队等它释放，表现为"一次快一次慢、连改几次就不动"。排查时容易一头扎进业务逻辑，其实根因在协议层。
  ✅ 长轮询/SSE 这类长连接必须配 HTTP/2（多路复用，不占槽位）。部署新环境或换反代时，把"h2 是否协商成功"当成上线检查项：`curl -o /dev/null -w '%{http_version}' --http2 <url>` 应返回 2。注意 `mod_http2` 不支持 mpm_prefork，且加载新模块要 `restart` 而不是 `reload`。
